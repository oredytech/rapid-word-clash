...
const Soldier: React.FC<SoldierProps> = ({
  isGamePlaying,
  currentInput,
  targetWord
}) => {
  ...
  return <>
      ...
      <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2 z-10">
        <div className={`soldier-container transition-all duration-200 ${isShooting ? 'scale-110' : 'scale-100'}`}>
          <div className="relative">
            ...
            {/* Effet de tir */}
            {isShooting && <div className="absolute top-0 left-1/2 transform -translate-x-1/2">
      ...
      {/* Rayons laser */}
      {laserBeams.map(laser => <div key={laser.id} className="absolute z-5 pointer-events-none" style={{
      left: '50%',
      bottom: '140px',
      transform: 'translateX(-50%)'
    }}>
          <svg className="absolute" width={Math.abs(laser.targetX - window.innerWidth / 2) + 50} height={Math.abs(laser.targetY - (window.innerHeight - 140)) + 50} style={{
        ...
        top: -(Math.abs(laser.targetY - (window.innerHeight - 140)) + 50)
            ...
            <line x1={laser.targetX > window.innerWidth / 2 ? 0 : Math.abs(laser.targetX - window.innerWidth / 2) + 50} y1={Math.abs(laser.targetY - (window.innerHeight - 140)) + 50} x2={laser.targetX > window.innerWidth / 2 ? Math.abs(laser.targetX - window.innerWidth / 2) + 50 : 0} y2={0} stroke="hsl(var(--accent))" strokeWidth="3" className="animate-pulse laser-beam" style={{
